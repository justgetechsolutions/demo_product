...
exports.getOrders = async(req, res) => {
    try {
        ...
        const ordersWithTableNumber = orders.map(order => {
            ...
            // Ensure billingInfo is included
            if (!o.billingInfo) {
                o.billingInfo = {};
            }

            console.log(`Order ${o._id} billingInfo:`, o.billingInfo);
            return o;
        });
...
exports.createOrder = async(req, res) => {
    try {
        ...
        // Fetch restaurant billing information
        ...
        const billingInfo = {
            restaurantName: restaurant ?.billingName || restaurant ?.name || '',
            address: restaurant ?.address || '',
            city: restaurant ?.city || '',
        ...
        console.log('BillingInfo to save:', billingInfo);
...
exports.createOrderPublic = async(req, res) => {
    try {
        ...
        const restaurant = await Restaurant.findById(restaurantId).select(
            'name billingName address city postalCode phone email gstNumber'
        );

        const billingInfo = restaurant ? {
            restaurantName: restaurant.billingName || restaurant.name || '',
            address: restaurant.address || '',
            city: restaurant.city || '',
            postalCode: restaurant.postalCode || '',
...
exports.createOrderPublicWithId = async(req, res) => {
    try {
        ...
        // Fetch restaurant to get slug and billing info
        const restaurant = await Restaurant.findById(restaurantId).select(
            'slug name billingName address city postalCode phone email gstNumber'
        );
    ...
    try {
        ...
        for (const item of orderItems) {
            await MenuItem.findByIdAndUpdate(
                item.menuItem, { $inc: { quantity: -item.qty } }
            );
        }
