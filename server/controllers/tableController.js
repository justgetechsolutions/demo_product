...
exports.updateTableStatus = async(req, res) => {
    try {
        ...
        const table = await Table.findOneAndUpdate({ _id: id, restaurantId: req.restaurantId },
            updateData, { new: true }
        ).populate('currentOrder');
...
exports.createOrderForTable = async(req, res) => {
    try {
        ...
        console.log('Creating order for table - Restaurant data:', {
            id: restaurant ?._id,
        ...
        console.log('Order created with billingInfo:', {
            _id: order._id,
        ...
        for (const item of items) {
            await MenuItem.findByIdAndUpdate(
                item.menuItem, { $inc: { quantity: -item.quantity } }
            );
        }
